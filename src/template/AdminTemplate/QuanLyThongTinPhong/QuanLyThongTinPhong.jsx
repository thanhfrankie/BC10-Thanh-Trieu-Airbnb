...
import { message as antdMessage, Switch, Upload, Button, Form, Input, Row, Col, Table, Modal ,message } from "antd";
...
const QuanLyThongTinPhong = () => {
  ...
  const uploadImageToApi = async (info, maPhong) => {
    try {
      ...
      const token = getToken();
      const response = await http.post(`/phong-thue/upload-hinh-phong?maPhong=${maPhong}`, formData, {
        headers: { token },
      });
  ...
  const columns = [
    { title: "ID", dataIndex: "id", key: "id", render: (id) => <span style={{ fontWeight: "bold" }}>{id}</span> },
    {
      title: "Hình Ảnh",
      dataIndex: "hinhAnh",
      key: "hinhAnh",
      render: (hinhAnh, record) => (
        <div style={{ display: "flex", alignItems: "center", marginRight: "40px" }}>
          <img src={hinhAnh} alt="Hình Ảnh" style={{ width: "100px", height: "50px", marginRight: "10px" }} />
    ...
    { title: "Tên Phòng", dataIndex: "tenPhong", key: "tenPhong", render: (tenPhong) => <span style={{ fontWeight: "bold" }}>{tenPhong}</span> },
    { title: "Mô Tả", dataIndex: "moTa", key: "moTa", render: (moTa) => <span style={{ color: "#2F4F4F" }}>{moTa.length > 100 ? `${moTa.substring(0, 100)}...` : moTa}</span> },
    { title: "Giá Tiền ($)", dataIndex: "giaTien", key: "giaTien", render: (giaTien) => <span style={{ fontWeight: "bold", color: "green" }}>{giaTien}</span> },
    {
      title: "Hành động",
      key: "action",
      render: (text, record) => (
        <span>
          <Button className="ml-1" type="default" onClick={() => handleEdit(record)}>Sửa</Button>
          <Button className=" ml-1 mb-1 mt-1" type="default" onClick={() => handleDelete(record)}>Xoá</Button>
          <Button className="ml-1" type="default" onClick={() => handleDetail(record)}>Xem chi tiết</Button>
        </span>
      ),
    },
  ];

  const filteredUsersData = usersData.filter((user) => user.tenPhong.toLowerCase().includes(searchKeyword.toLowerCase()));

  return (
    <div className="container">
      <div>
      <Button type="primary" onClick={handleOpenModal}><i className="fa-regular fa-circle-plus mr-1"></i> Thêm Thông Tin Phòng</Button>
...
<Modal
  ...
  footer={[
    ...
    <Button key="submit" type="primary" onClick={() => form.submit()}>{editingUser ? "Cập nhật" : "Thêm"}</Button>,
  ]}
>
  <Form
    form={form}
    ...
    <Row gutter={[8, 0]}>
      ...
      <Col span={8}>
        <Form.Item
          ...
          rules={[
            ...
            { pattern: /^[0-9]+$/, message: "Nhập vào chữ số." },
          ]}
        >
      ...
      <Col span={8}>
        <Form.Item
          ...
          label="Phòng khách"
          rules={[
            { required: true, message: "Nhập số lượng phòng" },
            { pattern: /^[0-9]+$/, message: "Nhập vào chữ số." },
          ]}
        >
          <Input prefix={<InsertRowRightOutlined />} placeholder="Số lượng phòng" />
        </Form.Item>
      </Col>
      <Col span={8}>
        <Form.Item
      ...
      <Col span={8}>
        <Form.Item
          ...
          label="Giường"
          rules={[
            { required: true, message: "Nhập số lượng giường" },
            { pattern: /^[0-9]+$/, message: "Nhập vào chữ số." },
          ]}
        >
          <Input prefix={<InboxOutlined />} placeholder="Số lượng giường" />
        </Form.Item>
      </Col>
      <Col span={8}>
        <Form.Item
        ...
        >
          <Input prefix={<InsertRowLeftOutlined />} placeholder="Số lượng phòng" />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="mayGiat" label="Máy Giặt">
          <Switch defaultChecked={editingUser ? editingUser.mayGiat : false} />
        </Form.Item>
      </Col>
    </Row>
    <Row gutter={[16, 0]}>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="banLa" label="Bàn Là">
          <Switch defaultChecked={editingUser ? editingUser.banLa : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="tivi" label="TV">
          <Switch defaultChecked={editingUser ? editingUser.tivi : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="dieuHoa" label="Điều hoà">
          <Switch defaultChecked={editingUser ? editingUser.dieuHoa : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="wifi" label="Wifi">
          <Switch defaultChecked={editingUser ? editingUser.wifi : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="bep" label="Bếp">
          <Switch defaultChecked={editingUser ? editingUser.bep : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="doXe" label="Đỗ xe">
          <Switch defaultChecked={editingUser ? editingUser.doXe : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="hoBoi" label="Hồ bơi">
          <Switch defaultChecked={editingUser ? editingUser.hoBoi : false} />
        </Form.Item>
      </Col>
      <Col xs={24} sm={12} md={8} lg={6}>
        <Form.Item name="banUi" label="Bàn ủi">
          <Switch defaultChecked={editingUser ? editingUser.banUi : false} />
        </Form.Item>
      </Col>
    </Row>
  </Form>
